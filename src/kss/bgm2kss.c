...
int KSS_isBGMdata(uint8_t *data, uint32_t size)
{
  ...
  {
    if((size>=0x60)&&!strncmp((const char *)(data+0x50),"BTO",3)) return 1;
    top = (data[2]<<8) + data[1];
    end = (data[4]<<8) + data[3];
    if(size==(8+end-top)&&data[7]<2) return 1;
    if(top==0xA5B7&&data[7]<2) return 1;
  }
...
int KSS_load_kinrou(const char *kinrou)
{
  ...
  if((fp=fopen(kinrou,"rb"))==NULL) return 1 ;
...
void KSS_get_info_bgmdata(KSS *kss, uint8_t *data, uint32_t size)
{
  ...
  if(size>0x60&&!strncmp((const char *)(data+0x50),"BTO",3))
  {
    ...
    offset = data[0x5D] + (data[0x5E]<<8) 
       - (data[0x59] + (data[0x5a]<<8)) + 7 ;
    for(i=0;i<255;i++)
    {
      if((offset+i)>=size) break ;
      if(data[offset+i]<0x20) break ;
    ...
    if(kss->extra!=NULL) strcpy((char *)kss->extra, extra) ;
  }
  else
  {
    kss->title[0] = '\0' ;
