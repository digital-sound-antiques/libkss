...
int KSS_isMPK103data(k_uint8 *data, k_uint32 size)
{
  ...
  if((size>=6)&&!strncmp((const char *)data,"MPK",3))
  {
    ...
    ver[1]=data[4];
    ver[2]=data[5];
    if(atoi(ver)<=103) return 1; else return 0;
  }
  else return 0 ;
}
...
int KSS_isMPK106data(k_uint8 *data, k_uint32 size)
{
  ...
  if((size>=6)&&!strncmp((const char *)data,"MPK",3))
  {
    ...
    ver[1]=data[4];
    ver[2]=data[5];
    if(103<atoi(ver)) return 1; else return 0;
  }
  else return 0 ;
}
...
int KSS_load_mpk106(const char *mpkdrv)
{
  ...
  if((fp=fopen(mpkdrv,"rb"))==NULL) return 1 ;
