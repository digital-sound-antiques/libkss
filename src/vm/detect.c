...
void 
LPDETECT_delete(LPDETECT *__this)
{
  if(__this) {
...
void 
LPDETECT_reset(LPDETECT *__this)
{
  int i;
...
LPDETECT_update(LPDETECT *__this, int32_t time_in_ms, int32_t match_second, int32_t match_interval)
{
  ...
  if (__this->m_loop_start>=0) return 1;

  if (time_in_ms - __this->m_current_time < match_interval) return 0;

  __this->m_current_time = time_in_ms;

  if (__this->m_bidx <= __this->m_blast) return 0;
