...
static BANK *BANK_new(k_uint32 type, k_uint8 *data)
{
  ...
  if(FAILED(bank = malloc(sizeof(BANK)))) return NULL ;
  ...
  if(data) memcpy(bank->body, data, size) ;
...
void MMAP_set_bank_attr(MMAP *mmap, k_uint32 slot, k_uint32 bank, k_uint32 attr)
{
  ...
  if(mmap->bank[slot][bank]) mmap->bank[slot][bank]->attr = attr ;
...
void MMAP_set_bank_data(MMAP *mmap, k_uint32 slot, k_uint32 bank, k_uint32 type, k_uint8 *data)
{
  ...
  if(mmap->bank[slot][bank]) MMAP_unset_bank(mmap, slot, bank) ; 
  ...
  { 
    int i ; 
    for(i=0;i<8;i++) MMAP_select_bank(mmap, i, mmap->current_slot[i], mmap->current_bank[i]) ;
  }
}
...
void MMAP_unset_bank(MMAP *mmap, k_uint32 slot, k_uint32 bank)
{
  ...
  {
    if(mmap->bank[slot][bank]->refc) mmap->bank[slot][bank]->refc-- ;
    else BANK_delete(mmap->bank[slot][bank]) ;
...
void MMAP_mirror_bank(MMAP *mmap, k_uint32 src_slot, k_uint32 src_bank, k_uint32 dst_slot, k_uint32 dst_bank)
{
  ...
  if(mmap->bank[dst_slot][dst_bank]) mmap->bank[dst_slot][dst_bank]->refc ++ ;
...
void MMAP_select_bank(MMAP *mmap, k_uint32 page, k_uint32 slot, k_uint32 bank)
{
  ...
  */
  ...
  {
    if(mmap->bank[slot][bank]->attr&BANK_READABLE) mmap->readmap[page] = mmap->bank[slot][bank]->body ;
    else mmap->readmap[page] = dummy_read_map ;
    if(mmap->bank[slot][bank]->attr&BANK_WRITEABLE) mmap->writemap[page] = mmap->bank[slot][bank]->body ;
    else mmap->writemap[page] = dummy_write_map ;
...
MMAP *MMAP_new()
{
  ...
  if(FAILED(mmap = malloc(sizeof(MMAP)))) return NULL ;
  memset(mmap, 0, sizeof(MMAP)) ;

  for(i=0;i<8;i++) MMAP_select_bank(mmap,i,0,i) ;
